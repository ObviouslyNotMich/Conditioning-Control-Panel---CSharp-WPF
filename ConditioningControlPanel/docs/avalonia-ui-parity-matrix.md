...
> - **Chaos / "Down the Rabbit Hole" run economy is now wired** — seeded lifetime-boon/upgrade/catalogues, SFX cue resolution, art loading from the effective assets root, narrative director shell, and bark notifications are all functional. Some WPF content (full narrative audio library, complete unlock-card pool) may still be parity placeholders; exercise end-to-end to confirm.
> - **Content-pack management (Assets), feature-card editors, and webcam tracking are now wired.** The remaining functional stubs are platform-specific (Linux/macOS frame capture, audio ducking) or parity stubs that also exist in WPF (Lab/BlinkTrainer engines).
>
...
- **Biggest remaining gaps:**
  ...
  4. **AvatarTube** — *(✅ companion switch, community prompts, attach/detach, and click reactions all wired.)* Structure restored: speech phrase system, AI chat, Circe emote engine, reaction hooks, drag/scale/floating/z-order, fullscreen detection, context-menu toggles, and emotive portrait system. `AvatarBorder` is hit-testable, `IBarkService.NotifyAvatarClicked()` triggers a random phrase giggle, AI chat double-click opens the input panel, and `AvatarEnabled` changes show/hide the tube. The window is created at startup when `AvatarEnabled`, content is scaled before the first `Show()` to prevent a `SizeToContent` blow-up, and the main window shifts right to keep the attached tube visible on screen.
...
| ChaosHudWindow.* | ChaosHudWindow.axaml + .axaml.cs + VM | ✅ | Win32 overlay ex-styles (`WS_EX_TOOLWINDOW | WS_EX_NOACTIVATE`) now applied; drop-shadow TODOs remain cosmetic only. |
...
| `EnhancementPlayerWindow.xaml.cs` | `EnhancementPlayerWindow.axaml.cs` | ✅ | LibVLC playback, drag-drop, file pickers, mini-timeline, Load URL wired; `EnhancementHostService` + `AvaloniaLibVlcTimeSource` bind the engine for live effect/rule playback. `AudioWaveformCache` is now wired so audio waveforms render. |
| `NewEnhancementDialog.xaml` | `NewEnhancementDialog.axaml` | ✅ | Visuals and bindings ported. |
| `NewEnhancementDialog.xaml.cs` | `NewEnhancementDialog.axaml.cs` | ✅ | Browse/create wired; open-file picker starts in `DeeperLastDirectory`; HypnoTube tutorial button mirrors WPF; interactive tutorial overlay is wired through `AvaloniaTutorialService`/`TutorialOverlay`. |
