...
///
/// Persistence: state is hydrated from / saved to
/// <c>{IAppEnvironment.ApplicationDataPath}/moderation-counter.json</c>.
/// Saves are fire-and-forget best-effort; failures don't break the counter pipeline.
/// </summary>
public sealed class ModerationCounter : IModerationCounter
{
    ...
    public ModerationCounter(IAppEnvironment appEnvironment)
    {
        if (appEnvironment == null) throw new ArgumentNullException(nameof(appEnvironment));
        _persistencePath = Path.Combine(appEnvironment.ApplicationDataPath, "moderation-counter.json");
    }
    ...
    /// <summary>
    /// Hydrates from <c>moderation-counter.json</c>. Called once at startup.
    /// Idempotent; safe if the file is missing or corrupt.
    /// </summary>
    ...
    private void SaveToDiskAsync()
    {
        ...
        // Serialize disk writes so a burst of hits cannot overwrite a newer snapshot
        // with an older one (the last queued write always re-reads current state).
        _ = Task.Run(async () =>
        {
