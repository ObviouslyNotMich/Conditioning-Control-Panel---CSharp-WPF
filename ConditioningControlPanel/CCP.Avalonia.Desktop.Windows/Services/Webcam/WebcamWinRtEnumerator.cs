...
namespace ConditioningControlPanel.Avalonia.Desktop.Windows.Services.Webcam
{
    ...
    {
        public static IReadOnlyList<WebcamDeviceEnumerator.WebcamDevice> Enumerate(ILogger<object>? logger)
        {
            ...
            try
            {
                ...
                {
                    logger?.LogWarning("WebcamWinRtEnumerator: FindAllAsync timed out");
                    return devices;
                }
                ...
                logger?.LogInformation("WebcamWinRtEnumerator: {Count} video-capture device(s) via WinRT", devices.Count);
            }
            catch (Exception ex)
            {
                logger?.LogWarning(ex, "WebcamWinRtEnumerator: enumeration threw");
            }
            return devices;
        }
    }
