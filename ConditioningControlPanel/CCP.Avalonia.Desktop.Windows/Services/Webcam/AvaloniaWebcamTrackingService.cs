...
// Disambiguate OpenCvSharp types from Core platform types.
...
namespace ConditioningControlPanel.Avalonia.Desktop.Windows.Services.Webcam
{
    ...
    //  stored value, new outbound data) MUST bump AvaloniaWebcamTrackingService.ConsentVersion
    ...
    /// </summary>
    public sealed class AvaloniaWebcamTrackingService : IWebcamService, IDisposable
    {
        /// <summary>
        ...
        /// <summary>
        /// Returns the ScreenInfo that matches the monitor the
        ...
        /// </summary>
        public ScreenInfo? GetCalibratedScreen()
        {
            var name = Calibration?.MonitorBounds?.DeviceName;
            if (string.IsNullOrEmpty(name)) return null;
            try
            {
                foreach (var screen in _screenProvider.GetAllScreens())
                {
            ...
            {
                _logger?.LogWarning(ex, "AvaloniaWebcamTrackingService.GetCalibratedScreen: enumeration failed");
            }
            return null;
        }
        ...
        /// </summary>
        public bool TryGetCalibratedBounds(out PixelRect bounds)
        {
        ...
        public event Action<CorePoint>? OnGazeMove;
        ...
        private readonly Queue<(DateTime Time, CorePoint ScreenPoint)> _gazeBuffer = new();
        ...
        private readonly ILogger<AvaloniaWebcamTrackingService> _logger;
        private readonly ILogger<WebcamCalibrationData>? _calibrationLogger;
        private readonly IScreenProvider _screenProvider;

        public AvaloniaWebcamTrackingService(
            IAppEnvironment environment,
            ISettingsService settings,
            ILogger<AvaloniaWebcamTrackingService> logger,
            IScreenProvider screenProvider,
            ILogger<WebcamCalibrationData>? calibrationLogger = null)
        {
            _environment = environment;
            _settings = settings;
            _logger = logger;
            _calibrationLogger = calibrationLogger;
        ...
        public bool Start()
        {
            lock (_stateLock)
            {
                ...
                if (!IsConsentCurrent())
                {
                    _logger?.LogInformation("AvaloniaWebcamTrackingService: Start() refused — consent not current (granted={Granted}, storedVersion={Stored}, currentVersion={Current})",
                        _settings?.Current?.WebcamConsentGiven, _settings?.Current?.WebcamConsentVersion, ConsentVersion);
                    return false;
                }
                if (_modelInitPermanentlyFailed)
                {
                    ...
                    _logger?.LogInformation("AvaloniaWebcamTrackingService: Start() refused — ONNX model load previously failed in a way that won't recover this session");
                ...
                {
                    _logger?.LogWarning("AvaloniaWebcamTrackingService: model files missing in Resources/Models/ (face_detection_short_range.onnx, blazeface_anchors.json, face_landmark.onnx, iris_landmark.onnx)");
                    SetState(WebcamTrackingState.Error);
                    return false;
                }
                ...
                _logger?.LogInformation("AvaloniaWebcamTrackingService: capture started ({W}x{H}, target {Fps} fps)",
        ...
        // ─────────────────────────────────────────────────────────────────────────
        //  IWebcamService implementation
        // ─────────────────────────────────────────────────────────────────────────

        public void StartTracking()
        {
            if (IsRunning) return;
            _logger.LogInformation("AvaloniaWebcamTrackingService: StartTracking requested");
            Task.Run(() => Start());
        }

        public void StopTracking() => Stop();

        public void Calibrate()
        {
            _logger.LogInformation("AvaloniaWebcamTrackingService: Calibrate() requested — calibration window wiring is a TODO");
        }

        public void TestTracker()
        {
            _logger.LogInformation("AvaloniaWebcamTrackingService: TestTracker() requested — tracker test window wiring is a TODO");
        }

        public void RefreshDevices()
        {
            var devices = EnumerateDevices();
            _logger.LogInformation("AvaloniaWebcamTrackingService: {Count} device(s) available after refresh", devices.Count);
        }
        ...
        public void RevokeConsent()
        {
            ...
            _logger?.LogInformation("AvaloniaWebcamTrackingService: consent revoked at {Time}", DateTime.UtcNow);
        }
        ...
        public void Stop()
        {
            ...
            if (thread != null)
            {
                ...
                {
                    _logger?.LogWarning("AvaloniaWebcamTrackingService: capture thread did not exit within 2s — extending wait");
                    if (!thread.Join(TimeSpan.FromSeconds(3)))
                    {
                        _logger?.LogError("AvaloniaWebcamTrackingService: capture thread did not exit within 5s total — leaving native handles alive to avoid disposal race");
                        lock (_stateLock)
                        {
                            SetState(WebcamTrackingState.Error);
                        }
            ...
            lock (_stateLock)
            {
                ...
                _logger?.LogInformation("AvaloniaWebcamTrackingService: stopped");
            }
        }
        ...
        {
            WebcamCalibrationData.DeleteIfExists(_environment, _calibrationLogger);
            Calibration = null;
        ...
        public void Dispose()
        {
            if (_disposed) return;
            _disposed = true;
            try { Stop(); } catch (Exception ex) { _logger?.LogWarning(ex, "AvaloniaWebcamTrackingService.Dispose: Stop() threw"); }
            _logger?.LogInformation("AvaloniaWebcamTrackingService: disposed");
        }
        ...
        {
            var devices = WebcamDeviceEnumerator.Enumerate(_logger);
            if (devices.Count == 0)
            {
                ...
                var winrt = WebcamWinRtEnumerator.Enumerate(_logger);
                if (winrt.Count > 0)
                {
                    _logger?.LogInformation("AvaloniaWebcamTrackingService: DirectShow found 0 devices; using WinRT/MF fallback — {Count} device(s): {Names}",
                        winrt.Count, string.Join(" | ", winrt.Select(d => $"[{d.Index}] {d.Name}")));
                    return winrt;
                }
                _logger?.LogInformation("AvaloniaWebcamTrackingService: no video-capture devices found via DirectShow or WinRT enumeration");
            }
            else
            {
                _logger?.LogInformation("AvaloniaWebcamTrackingService: {Count} video-capture device(s) detected: {Names}",
        ...
        private bool TryOpenCamera()
        {
            ...
            if (!openTask.Wait(TimeSpan.FromSeconds(CameraOpenTimeoutSeconds)))
            {
                _openAbandoned = true;
                _logger?.LogWarning(
                    "AvaloniaWebcamTrackingService: camera open timed out after {Seconds}s — driver may be hung or the device is held exclusively by another app",
        ...
        private VideoCapture? OpenCaptureCore()
        {
            ...
            try
            {
                int configured = _settings?.Current?.WebcamDeviceIndex ?? -1;
                ...
                if (devices.Count > 0)
                {
                    ...
                    {
                        _logger?.LogWarning(
                            "AvaloniaWebcamTrackingService: configured device index {Configured} is out of range ({Count} devices present); falling back to 0",
                            deviceIndex, devices.Count);
                        deviceIndex = 0;
                    }
                    detectedName = devices[deviceIndex].Name;

                    string savedName = _settings?.Current?.WebcamDeviceName ?? "";
                    if (!string.IsNullOrEmpty(savedName) && !string.Equals(savedName, detectedName, StringComparison.Ordinal))
                    {
                        _logger?.LogWarning(
                            "AvaloniaWebcamTrackingService: device at index {Index} is '{Detected}', but settings remembered '{Saved}' — enumeration order may have shifted",
                            deviceIndex, detectedName, savedName);
                    }
                }
                else if (configured >= 0)
                {
                    _logger?.LogWarning("AvaloniaWebcamTrackingService: no devices enumerated but settings remember index {Index} — opening anyway", configured);
                }
                ...
                for (int attemptIdx = 0; attemptIdx < CaptureAttempts.Length; attemptIdx++)
                {
                    ...
                    if (cap != null)
                    {
                        _logger?.LogInformation(
                            "AvaloniaWebcamTrackingService: device {Index} ('{Name}') opened successfully via {Api}{Fourcc}",
                ...
                if (_nativeRuntimeMissing)
                {
                    ...
                    _logger?.LogError(
                        "AvaloniaWebcamTrackingService: OpenCV native runtime (OpenCvSharpExtern.dll) failed to load for device index {Index} ('{Name}') — the Microsoft Visual C++ Redistributable (x64) is almost certainly missing. Install it from https://aka.ms/vs/17/release/vc_redist.x64.exe",
                ...
                else if (anyOpened)
                {
                    ...
                    _logger?.LogWarning(
                        "AvaloniaWebcamTrackingService: device index {Index} ('{Name}') opened but produced no usable frame on any backend",
                        deviceIndex, detectedName);
                    SetState(WebcamTrackingState.CameraInUse);
                }
                else
                {
                    _logger?.LogWarning(
                        "AvaloniaWebcamTrackingService: VideoCapture.Open returned false on all backends for device index {Index} ('{Name}')",
                        deviceIndex, detectedName);
            ...
            {
                _logger?.LogWarning(ex, "AvaloniaWebcamTrackingService: OpenCaptureCore threw");
                SetState(WebcamTrackingState.Error);
                return null;
            }
        }
        ...
        private VideoCapture? TryOpenWithBackend(int deviceIndex, string detectedName, VideoCaptureAPIs api, string apiName, string? fourcc, ref bool anyOpened)
        {
            ...
            _logger?.LogInformation("AvaloniaWebcamTrackingService: opening device index {Index} ('{Name}') with {Api}", deviceIndex, detectedName, label);
            ...
            try
            {
                ...
                if (!cap.IsOpened())
                {
                    _logger?.LogInformation("AvaloniaWebcamTrackingService: {Api} open failed for index {Index}", label, deviceIndex);
                    cap.Dispose();
                    return null;
                }
                anyOpened = true;
                ...
                while (DateTime.UtcNow < probeDeadline)
                {
                    ...
                    if (cap.Read(probe) && !probe.Empty())
                    {
                        ...
                        {
                            _logger?.LogInformation(
                                "AvaloniaWebcamTrackingService: device {Index} ('{Name}') via {Api} probe ok ({W}x{H}, mean={Mean:F1}, maxStdDev={Std:F1}, temporalDelta={Td:F1}, reads={Reads})",
                        ...
                        _logger?.LogDebug(
                            "AvaloniaWebcamTrackingService: device {Index} via {Api} degenerate probe frame (maxStdDev={Std:F1} < {Min}, temporalDelta={Td:F1} < {MinTd})",
                ...
                _logger?.LogWarning(
                    "AvaloniaWebcamTrackingService: device index {Index} ('{Name}') opened via {Api} but produced no usable frame in {Budget}ms ({Reads} reads) — trying next attempt",
            ...
            catch (Exception ex)
            {
                ...
                _logger?.LogWarning(ex, "AvaloniaWebcamTrackingService: TryOpenWithBackend({Api}) threw for index {Index}", label, deviceIndex);
        ...
        private bool TryLoadModels(string faceModelPath, string faceAnchorsPath, string meshModelPath, string irisModelPath)
        {
            ...
            try
            {
                ...
                _faceMesh = mesh;
                _irisDetector = iris;
                _logger?.LogInformation("AvaloniaWebcamTrackingService: BlazeFace + FaceMesh + Iris loaded");
                return true;
            }
            catch (Exception ex)
            {
                ...
                if (isEtwFailure)
                {
                    ...
                    _logger?.LogWarning(
                        "AvaloniaWebcamTrackingService: ONNX Runtime cannot initialize because Windows Event Tracing (ETW) is restricted on this machine. This is usually caused by Group Policy, antivirus, or a disabled Event Log service. Webcam tracking will remain unavailable until that's resolved.");
                }
                else
                {
                    _logger?.LogWarning(ex, "AvaloniaWebcamTrackingService: failed to load detection models");
                }
        ...
        {
            _logger?.LogInformation("AvaloniaWebcamTrackingService: capture loop entered");
            ...
            try
            {
                while (!_stopRequested)
                {
                    ...
                    if (!capture.Read(frame) || frame.Empty())
                    {
                        ...
                        {
                            _logger?.LogWarning("AvaloniaWebcamTrackingService: {N} consecutive read failures -- stopping", consecutiveReadFails);
                            SetState(WebcamTrackingState.Error);
                            break;
                        }
                        Thread.Sleep(20);
                    ...
                    {
                        _logger?.LogWarning(ex, "AvaloniaWebcamTrackingService: ProcessFrame threw");
                        Thread.Sleep(50);
                    }
                    ...
                    if (processedFrames % 300 == 0)
                    {
                        ...
                        _logger?.LogDebug("AvaloniaWebcamTrackingService: {Frames} frames processed, ~{Fps:F1} fps", processedFrames, fps);
                    }
                }
            }
            catch (Exception ex)
            {
                _logger?.LogWarning(ex, "AvaloniaWebcamTrackingService: capture loop terminated by exception");
                SetState(WebcamTrackingState.Error);
            }
            finally
            {
                _logger?.LogInformation("AvaloniaWebcamTrackingService: capture loop exited after {Frames} frames", processedFrames);
            }
        }
        ...
        private void UpdateHeadPose(float[][] landmarks, int frameW, int frameH)
        {
            ...
            {
                _headPoseValid = false;
                _logger?.LogDebug("UpdateHeadPose failed: {Error}", ex.Message);
            }
        }
        ...
        private CorePoint? ProjectGazeToScreen(double irisDx, double irisDy)
        {
        ...
        private void UpdateLongStareHeuristic(CorePoint point)
        {
            ...
            var stareCenter = new CorePoint(cx, cy);
        ...
        private void UpdateBlinkState(double earL, double earR)
        {
            ...
            else if (!nowClosed && _eyesClosed && _eyesClosedAt.HasValue)
            {
                ...
                _logger?.LogDebug(
                    "AvaloniaWebcamTrackingService: blink {Outcome} (closed for {Ms:F0}ms, baseline EAR={Base:F3}, min EAR during closure={Min:F3}, ratio={Ratio:F2}× baseline)",
        ...
        private void MaybeLogBlinkDiag(DateTime now, double avgEar)
        {
            ...
            // per-blink log above.
            _logger?.LogDebug(
                "AvaloniaWebcamTrackingService: blink-diag baseline={Base:F3} closedThr={CT:F3} openThr={OT:F3} winMin={WMin:F3}({WMinR:P0}) winMax={WMax:F3}({WMaxR:P0}) nearMiss={NM} state={State} blinks={N} samples={S}",
            ...
            else if (nowOpen)
            {
                ...
                if (_mouthOpenedAt.HasValue && _lastMouthOpenAt < _mouthOpenedAt.Value)
                {
                        ...
                        && (now - _lastMouthOpenAt).TotalMilliseconds >= MouthCooldownMs)
                    {
                        ...
                        _logger?.LogDebug(
                            "AvaloniaWebcamTrackingService: mouth-open #{N} FIRED (open for {Ms:F0}ms, baseline MAR={Base:F3}, max MAR during open={Max:F3}, ratio={Ratio:F2}× baseline)",
        ...
        private void MaybeLogMouthDiag(DateTime now)
        {
            ...
            _logger?.LogDebug(
                "AvaloniaWebcamTrackingService: mouth-diag baseline={Base:F3} openThr={OT:F3} closeThr={CT:F3} winMin={WMin:F3} winMax={WMax:F3}({WMaxR:P0}) state={State} mouthOpens={N} samples={S}",
            ...
            else if (nowOut)
            {
                ...
                if (_tongueOutSince.HasValue && _lastTongueOutAt < _tongueOutSince.Value)
                {
                        ...
                        && (now - _lastTongueOutAt).TotalMilliseconds >= TongueCooldownMs)
                    {
                        ...
                        _logger?.LogDebug(
                            "AvaloniaWebcamTrackingService: tongue-out #{N} FIRED (visible for {Ms:F0}ms, max ratio={Max:P0})",
        ...
        private void MaybeLogTongueDiag(DateTime now)
        {
            ...
            // Debug, not Information — see blink-diag for reasoning.
            _logger?.LogDebug(
                "AvaloniaWebcamTrackingService: tongue-diag winMaxRatio={Max:P0} state={State} fires={N} frames={F} | per-class share: tongue={T:P0} teeth={E:P0} shadow={S:P0} other={O:P0}",
