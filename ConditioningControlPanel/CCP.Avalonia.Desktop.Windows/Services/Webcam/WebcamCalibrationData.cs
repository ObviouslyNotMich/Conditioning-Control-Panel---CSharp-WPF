...
namespace ConditioningControlPanel.Avalonia.Desktop.Windows.Services.Webcam
{
    ...
    /// in the user data folder (see <see cref="IAppEnvironment.UserDataPath"/>)/webcam-calibration.json.
    ///
    ...
    public class WebcamCalibrationData
    {
        ...
        public static WebcamCalibrationData? Load(IAppEnvironment environment, ILogger<WebcamCalibrationData>? logger)
        {
            try
            {
            ...
            {
                logger?.LogWarning(ex, "WebcamCalibrationData: failed to load");
                return null;
            }
        }

        public void Save(IAppEnvironment environment, ILogger<WebcamCalibrationData>? logger)
        {
            try
            {
                Directory.CreateDirectory(environment.UserDataPath);
                var json = JsonConvert.SerializeObject(this, Formatting.Indented);
                File.WriteAllText(GetFilePath(environment), json);
            }
            catch (Exception ex)
            {
                logger?.LogWarning(ex, "WebcamCalibrationData: failed to save");
            }
        }

        public static void DeleteIfExists(IAppEnvironment environment, ILogger<WebcamCalibrationData>? logger)
        {
            try
            {
                var path = GetFilePath(environment);
            ...
            {
                logger?.LogWarning(ex, "WebcamCalibrationData: failed to delete");
            }
        }

        /// <summary>
        /// Returns a shallow clone of this calibration with <see cref="RuntimeOffset"/>
        /// replaced. Use this (and re-publish via <see cref="AvaloniaWebcamTrackingService.SetRuntimeOffset"/>)
    ...
    /// projected through the symmetric form (see AvaloniaWebcamTrackingService.ProjectGazeToScreen).
    /// </summary>
